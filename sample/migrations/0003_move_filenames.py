...
class Migration(DataMigration):
	def forwards(self, orm):
		...
		for file in orm.FileSample.objects.all().order_by('submission_time'):
			on_obj, created = orm.OtherFilename.objects.get_or_create(filename = file.filename)
			if created:
				on_obj.first_seen = file.submission_time
			on_obj.save()
			file.other_name.add(on_obj)

	def backwards(self, orm):
		"Write your backwards methods here."
		raise RuntimeError("Can't bring this migration backwards")
	...
	models = {
		...
		'sample.filesample': {
			...
			'other_name': ('django.db.models.fields.related.ManyToManyField', [],
							   {'to': "orm['sample.OtherFilename']", 'symmetrical': 'False'}),
